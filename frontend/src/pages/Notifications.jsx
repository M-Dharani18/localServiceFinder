...
import { Bell, Check, X, Settings, Clock, AlertCircle, CheckCircle, Info, MessageSquare, Star, Calendar } from 'lucide-react'
...
export default function Notifications() {
  ...
  const fetchNotifications = async () => {
    ...
    try {
      const data = await getUserNotificationsApi(user.id)
  ...
  const filters = [
    ...
    { id: 'booking', label: 'Bookings', count: notifications.filter(n => n.type?.includes('BOOKING')).length },
  ]
  ...
  const getNotificationIcon = (type) => {
    if (!type) return <Bell className="w-5 h-5 text-gray-600" />
    
    if (type.includes('BOOKING_CREATED') || type.includes('NEW_BOOKING')) {
      return <Calendar className="w-5 h-5 text-blue-600" />
    }
    if (type.includes('BOOKING_CONFIRMED')) {
      return <CheckCircle className="w-5 h-5 text-green-600" />
    }
    if (type.includes('BOOKING_COMPLETED')) {
    ...
    if (type.includes('promotion')) {
      return <Bell className="w-5 h-5 text-fuchsia-600" />
    }
  ...
  const filteredNotifications = notifications.filter(notif => {
    ...
    if (activeFilter === 'important') return notif.isImportant
    if (activeFilter === 'booking') return notif.type?.includes('BOOKING')
    return true
  })
  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="px-4 py-2 rounded-lg border border-blue-200 text-blue-700 font-medium hover:bg-blue-50 transition-colors"
          ...
          <div className="divide-y divide-gray-100">
            {filteredNotifications.map(notification => (
              <div 
                ...
                className={`
                  p-6 hover:bg-gray-50/50 transition-colors
                  ${!notification.isRead ? 'bg-gradient-to-r from-purple-50/50 to-fuchsia-50/50' : ''}
                `}
              >
                <div className="flex items-start gap-4">
                  <div className="flex-shrink-0 mt-1">
      ...
      <div className="mt-4 text-center text-sm text-gray-500">
        <p>🔄 Notifications refresh automatically every 30 seconds</p>
      </div>
    </div>
  )
}